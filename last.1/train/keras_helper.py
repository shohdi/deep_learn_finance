...
class KerasHelper:
    def convNetBuild(self,input_shape):
        model = Sequential();
        '''
        model.add(Conv1D(filters=240,kernel_size=12,activation="relu",input_shape=input_shape));
        model.add(GlobalMaxPooling1D());
        model.add(Conv1D(filters=480,kernel_size=12,activation="relu"));
        model.add(GlobalMaxPooling1D());
        model.add(Conv1D(filters=960,kernel_size=12,activation="relu"));
        model.add(GlobalMaxPooling1D());
        model.add(Conv1D(filters=1920,kernel_size=12,activation="relu"));
        model.add(GlobalMaxPooling1D());
        ...
        model_file = os.path.join(FLAGS.outputDir,'conv3.h5');
